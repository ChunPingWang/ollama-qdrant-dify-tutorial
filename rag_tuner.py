...
from rag_config import (
    BASE_DIR, PDF_PATH, BM25_PATH, COLLECTION_NAME,
    EMBED_MODEL, EMBED_DIM,
    QDRANT_HOST, QDRANT_PORT,
    get_qdrant_client, expand_query, tokenize,
)
...
def build_index(chunk_size: int, chunk_overlap: int) -> tuple[object, dict]:
    """重建向量資料庫與 BM25 索引，回傳 (client, bm25_data)。"""
    import fitz
    ...
    return client, bm25_data


def hybrid_search_with_params(
    question: str, client, bm25_data: dict,
...
def run_quick_tuning():
    ...
    print(f"向量資料庫: {info.points_count} 個文字塊")
    ...
    for i, (top_k, vec_w, rrf_k) in enumerate(combos):
        ...
        metrics = eval_params(client, bm25_data, top_k, vec_w, rrf_k)
        all_results.append({
...
def run_full_tuning():
    ...
    for chunk_size, chunk_overlap in chunk_combos:
        ...
        print(f"{'='*50}")
        t0 = time.time()
        client, bm25_data = build_index(chunk_size, chunk_overlap)
        info = client.get_collection(COLLECTION_NAME)
        n_chunks = info.points_count
